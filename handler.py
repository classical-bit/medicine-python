...
drug_data = dict(NAME='', COMPANY='', COMPOSITION='', PRESCRIPTION='', DRUGINFO='', COST='', QUANTITY='', USES='',
                 SIDEEFFECTS='', HOWTOUSE='', HOWITWORKS='', EXPERTADVICE=[], SUBSTITUTE=[])

for alpha in range(0, 1):
    url_base_alpha = url_base + alphabets[alpha]
    page_soup = get_soup(url_base_alpha)

    for link in page_soup.find_all("a", {"class": "_1hWpa"}):
    ...
    for page in range(0, total_pages):
        ...
        for drug_address in range(0, len(drugs_list)):
            if list(drugs_list[drug_address])[1] == 'd':
                ...
                drug_data['NAME'] = page_soup.find('h1', {"class": "adCs-"}).string
                drug_data['COMPANY'] = page_soup.find('div', {"class": "_39Abk"}).string
                drug_data['COMPOSITION'] = page_soup.find('div', {"class": "saltInfo _2-9Vh"}).a.string
                drug_data['PRESCRIPTION'] = page_soup.find('div',
                                                          {"class": "_21T_g"}).text
                drug_data['DRUGINFO'] = page_soup.find('div', {"class": "_2qdTY"}).text
                drug_data['COST'] = page_soup.find('div', {"class": "dj2lv"}).text
                drug_data['QUANTITY'] = page_soup.find('div', {"class": "_2LGBX"}).text
                drug_data['USES'] = page_soup.find('div', {"class": "_38C-l"}).text
                drug_data['SIDEEFFECTS'] = page_soup.find('div', {"class": "_2yk_W"}).text
                drug_data['HOWTOUSE'] = page_soup.find('div', {"class": "_3mfDy"}).text
                drug_data['HOWITWORKS'] = page_soup.find('div', {"class": "_232QU"}).text
                ...
                for anchors in page_soup.find_all('div', {"class": "row _1wbMv"}):
                    temp = []
                    temp = [anchors.a["href"], anchors.find("div", {'class': 'PH8Al'}).text,
                            anchors.find("div", {'class': '_2X-vB'}).text,
                            anchors.find("div", {'class': 'MIbLo'}).text]
